...
class RetrievePaymentOrderStatusServiceTest {
    ...
    void shouldRetrievePaymentOrderStatusSuccessfully() {
        ...
        // Act
        PaymentStatus result = service.retrieveStatus(paymentOrderReference);

        // Assert
        Assertions.assertThat(result).isNotNull();
        Assertions.assertThat(result).isEqualTo(PaymentStatus.INITIATED);
    }
    ...
    void shouldThrowWhenOrderNotFound() {
        ...
        // Act & Assert
        Assertions.assertThatThrownBy(() -> service.retrieveStatus(paymentOrderReference))
            .isInstanceOf(PaymentOrderNotFoundException.class)
            .hasMessageContaining("Payment order with reference 'PO-NOTFOUND' not found");
    }

    @Test
    @DisplayName("Should return current status of the order")
    void shouldReturnCurrentStatusOfTheOrder() {
        // Arrange
        String paymentOrderReference = "PO-TEST123456";
        PaymentOrder order = createValidPaymentOrder();
        ...
        // Assert
        Assertions.assertThat(result).isEqualTo(PaymentStatus.PENDING);
    }
}
